...
$(document).ready(function () {
	...
	$("#product-search").on("input", function () {
		...
		searchTimeout = setTimeout(() => {
			$.get(
				`../../api/catalog?search=${encodeURIComponent(query)}`,
				function (products) {
	...
	function renderSearchResults(products) {
		...
		if (!Array.isArray(products) || products.length === 0) {
			$results.append(
				'<div class="search-item text-muted p-3" role="option">Nenhum produto encontrado.</div>',
			);
		} else {
			products.forEach((p) => {
				const $item = $(`
	...
	function addItem(product) {
		...
		const existing = quotationItems.find(
			(item) =>
				item.merc === product.merc && item.digito === product.digito,
		);
	...
	window.updatePrice = function (merc, digito, newPrice) {
		...
		if (item) {
			const price =
				parseFloat(newPrice.toString().replace(",", ".")) || 0;
			item.price = price;
			saveAndRender();
		}
	};
	...
	$("#clear-quotation").on("click", function () {
		if (
			...
			confirm("Deseja realmente limpar toda a cotação?")
		) {
			quotationItems = [];
	...
	function renderQuotation() {
		...
		if (quotationItems.length === 0) {
			$body.append(
				'<tr><td colspan="9" class="text-center py-5 text-muted italic">Nenhum item adicionado à cotação.</td></tr>',
			);
	...
	$("#generate-pdf").on("click", function () {
		if (quotationItems.length === 0) {
			Swal.fire(
				"Cotação Vazia",
				"Adicione pelo menos um item para gerar o faturamento.",
				"warning",
			);
			return;
		}
		...
		$("#pdf-client").text(
			$("#client-name-input").val() || "CLIENTE NÃO INFORMADO",
		);
		...
		quotationItems.forEach((item) => {
			...
			$pdfBody.append(`
                <tr style="font-size: 10px;">
                    <td style="padding: 8px;">${item.merc}</td>
                    <td style="padding: 8px;">${item.digito}</td>
                    <td style="padding: 8px; text-transform: uppercase;">${item.descricao}</td>
                    <td style="padding: 8px;">${item.embalagem}</td>
                    <td style="padding: 8px; text-align: center; font-weight: bold;">${item.quantity}</td>
                    <td style="padding: 8px; text-align: center;">R$ ${item.price.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</td>
                    <td style="padding: 8px; text-align: center; font-weight: bold;">R$ ${rowTotal.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</td>
                    <td class="text-left font-barcode" style="font-size: 32px; padding: 5px; padding-top: 10px;">*${item.merc}${item.digito}*</td>
                </tr>
            `);
		});
		...
		setTimeout(() => {
			html2canvas(document.querySelector("#pdf-export-container"), {
				scale: 3, // Sufficient for barcode reading with 32px font
				useCORS: true,
				logging: false,
				backgroundColor: "#ffffff",
			}).then((canvas) => {
				// Use JPEG with 0.7 compression to reduce size from 10MB+ to ~500KB
